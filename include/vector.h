...
/**
 * Initializes <vec> to be used as Vector of elements of size <size>.
 * If <vec> is NULL, memory is allocated. Note that in this case, the
 * caller must ensure the memory is deallocated after use.
...
/**
 * Appends <element> to <vec> by copying <element_size> bytes from <element> into
 * the vector. Note that this is the only function to grow a vector.
 * Returns the pointer to the element in the vector.
 */
void* VecAppend(Vector* vec, void* element);

/**
 * Returns a pointer to the n-th element of <vec> or NULL, if the index is out of range.
 * If n is negative, the index is counted from the end of the vector.
 * If <element> is not NULL, the element is also copied there.
 */
void* VecGet(Vector* vec, int n, void* element);
